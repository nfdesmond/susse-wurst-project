...
class StoreLookupGUI:
    def __init__(self, root, cxn):
        ...
        self.logo = tk.PhotoImage(file='sw_storelookup_logo.png')

        logo_label = ttk.Label(topframe, image=self.logo, background='#0B769F')
        ...
        store_list_label = ttk.Label(mainframe, width=20, text='Select a store number', font=('TkDefaultFont', 15, 'bold'))
        ...
        get_store_button = ttk.Button(mainframe, text='Search', style='Button.TButton', command=self.set_store_info)
    ...
    def go_to_email(self, email_address):
        webbrowser.open_new_tab(f"mailto:{email_address}")
        return None